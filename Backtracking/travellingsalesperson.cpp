...
void tspBacktracking(int current, vector<int> &path, int visited[], int cost)
{
    if (path.size() == n)
    {
    ...
    for (int i = 0; i < n; i++)         //input connected graph 
    {
        if (!visited[i])                 //traverse all points
        {
            visited[i] = 1;
            path.push_back(i);

            tspBacktracking(i, path, visited, cost + graph[current][i]);
...
int main()
{
    ...
    tspBacktracking(src, path, visited, 0);
